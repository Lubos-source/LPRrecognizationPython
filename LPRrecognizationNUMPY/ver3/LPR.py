...
def LPRmain(fileName = '1', imgShow = True):
    ...
    text = pytesseract.image_to_string(Cropped, lang='eng', config='--psm 10 -c tessedit_char_whitelist=ABCDEFHIJKLMNPRSTUVXYZ0123456789')[:-2]                                                                                           
    #config --psm10 je single character recognition (moznosti nastaveni --psm lze najit napsanim do konzole "tesseract --help-psm") mozno dodat --oem 1 (Neural Nets) 
    #[:-2] odebere posledni dva symboly (kvuli \n a neznamemu symbolu)
